...
function print_var($var, $head = 'PrintVar') {
    if (defined('DISABLE_PRINT_VAR'))
        return;

    if (PrintVarSettings::$RunOnlyInDebugMode) {
        if (!isset($_GET[PrintVarSettings::$DebugParamName]))
            return;
        if ($_GET[PrintVarSettings::$DebugParamName] != PrintVarSettings::$DebugParamValue)
            return;
    }
...
class PrintVarService {
    ...
    public static function Init() {
        if (!self::$service)
            self::$service = new PrintVarService();
        return self::$service;
    }
    ...
    private function GetScript() {
        return '
            (function(undefined){
                var jQuerySource = "https://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js";
    ...
    private function PrintObject($var, $name = null, $separator = '=', $namePrefix = null) {
        ...
        if ($canPrintValue) {
            ...
            foreach ($methods as $method) {
                ...
                if (in_array($name, $this->arMethodsExcept))
                    continue;
